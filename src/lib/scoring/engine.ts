...
export function evaluateClaim(
  ...
  const rawClaimNumber =
    ('Claim Number' in claimData && claimData['Claim Number'] !== undefined
      ? claimData['Claim Number']
      : undefined) ?? claimData['Claim number'];

  const claimNumber =
    rawClaimNumber === undefined || rawClaimNumber === null
      ? 'N/A'
      : typeof rawClaimNumber === 'string'
        ? rawClaimNumber
